import time
import pytest
from app.models.domain import Configuration, BillOfMaterials, BOMItem, BOMOrigin
...
def generate_large_configuration(num_components: int) -> Configuration:
    ...
    config.bill_of_materials = BillOfMaterials(items=items, total_components=num_components)
    return config
