...
from app.models.domain import (
    ProductCatalogue, CatalogMetadata, Component, FeatureOption,
    Rule, RuleAction, RuleTriggerType, Dependency, DependencyType,
    Configuration, ConfigurationStatus, RuleContext, DependencyResolutionContext,
...
class MockDepRepo:
    def __init__(self, data): self._data = data
    def get_all(self): return self._data
    def get_outgoing(self, node_id): return [d for d in self._data if d.source_id == node_id]

@pytest.fixture
def large_catalogue():
    # 1000 components, 1000 options, 1000 rules, 1000 dependencies
    components = [Component(id=f"C_{i}", category="Cabin", name=f"Comp {i}", unit="pcs") for i in range(1000)]
    options = [FeatureOption(id=f"O_{i}", feature_id="F1", display_name=f"Opt {i}", internal_value=str(i)) for i in range(1000)]
    ...
    for i in range(1000):
        ...
        if i < 999:
            deps.append(Dependency(id=f"D_{i}", source_id=f"C_{i}", target_id=f"C_{i+1}", dependency_type=DependencyType.REQUIRES))

    return ProductCatalogue(
        metadata=CatalogMetadata(
            catalogue_version="v_bench", schema_version="1.0",
            created_date="2026-07-08T00:00:00Z",
            last_updated="2026-07-08T00:00:00Z",
            prototype_version="1.0",
        ),
        categories=[], feature_groups=[], features=[],
        components=components, feature_options=options, mappings=[],
        dependencies=deps
    )
...
def test_performance_benchmark(large_catalogue):
    ...
    class FastAction:
        def validate_payload(self, p): pass
    ...
    r_ctx = RuleContext(configuration=config, catalogue=large_catalogue, correlation_id="b1", execution_timestamp="ts", trigger_type=RuleTriggerType.ON_SELECTION)
    ...
    d_ctx = DependencyResolutionContext(configuration=config, catalogue=large_catalogue, report=d_rep, correlation_id="b2", execution_timestamp="ts")
    ...
    print(f"[BENCHMARK] Dependency Engine (1000 nodes, deep traversal): {dep_time_ms:.2f} ms")
    
    # Simple assert to ensure it actually ran
    assert len(r_rep.executed_rules) == 100
    assert d_res_rep.metrics.resolved_nodes == 1000  # since C_0 -> C_1 ... -> C_999 is a single chain, resolving any triggers the rest
