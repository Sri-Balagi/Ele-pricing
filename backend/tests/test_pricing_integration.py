...
def integrated_catalogue():
    pricing_cat = PricingCatalogue(
        ...
        tax_configuration=TaxConfiguration(
            enabled=True, tax_name="VAT", rate=Decimal("20.0")
        ),
        pricing_records=[
            PricingRecord(
                entity_id="CAT_A", entity_type="CATEGORY", price=Decimal("5000")
            ),
            PricingRecord(
                entity_id="OPT_USER", entity_type="FEATURE_OPTION", price=Decimal("500")
            ),
            PricingRecord(
                entity_id="COMP_M2", entity_type="COMPONENT", price=Decimal("100")
            ),
            PricingRecord(
                entity_id="COMP_M3", entity_type="COMPONENT", price=Decimal("200")
            ),
            PricingRecord(
                entity_id="COMP_FINAL", entity_type="COMPONENT", price=Decimal("300")
            ),
        ],
    )
...
def test_full_pipeline_m2_m3_m4(integrated_catalogue, base_config):
    ...
    base_config.bill_of_materials = BillOfMaterials(
        items=[
            BOMItem(component_id=comp, quantity=1)
            for comp in base_config.resolved_components
        ]
    )
