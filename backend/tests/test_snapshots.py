...
def test_dependency_report_snapshot():
    ...
    expected_keys = {
        "configuration_id", "metrics", "components_added", "options_added",
        "conflicts", "warnings", "cycles_detected", "execution_order", "summary"
    }
    assert set(data.keys()) == expected_keys
...
def test_rule_report_snapshot():
    ...
    expected_keys = {
        "configuration_id", "trigger", "executed_rules", "skipped_rules",
        "failed_rules", "execution_time_ms", "summary", "metrics"
    }
    assert set(data.keys()) == expected_keys
