...
def adversarial_catalogue():
    return ProductCatalogue(
        metadata=CatalogMetadata(
            catalogue_version="v_adv", schema_version="1.0",
        ...
        dependencies=[
            # Cycle: C1 -> C2 -> C3 -> C1
            Dependency(id="D1", source_id="C1", target_id="C2", dependency_type=DependencyType.REQUIRES),
            Dependency(id="D2", source_id="C2", target_id="C3", dependency_type=DependencyType.REQUIRES),
            Dependency(id="D3", source_id="C3", target_id="C1", dependency_type=DependencyType.REQUIRES),
            # Missing component: C3 -> C_MISSING
            Dependency(id="D4", source_id="C3", target_id="C_MISSING", dependency_type=DependencyType.REQUIRES)
        ],
        rules=[],
    )
...
def test_dependency_engine_cycle_detection(adversarial_catalogue):
    ...
    valid_deps = [d for d in adversarial_catalogue.dependencies if d.target_id != "C_MISSING"]
    ...
    ctx = DependencyResolutionContext(
        configuration=config, catalogue=adversarial_catalogue, report=report,
        correlation_id="corr-cycle", execution_timestamp="ts"
    )
