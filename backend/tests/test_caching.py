...
def base_catalogue():
    return ProductCatalogue(
        metadata=CatalogMetadata(
            catalogue_version="v1.0", schema_version="1.0",
...
def test_cache_hit_on_same_version(base_catalogue):
    ...
    registry._data.append(Dependency(id="D2", source_id="C2", target_id="C1", dependency_type=DependencyType.REQUIRES))
...
def test_cache_invalidation_on_version_bump(base_catalogue):
    ...
    base_catalogue.components.append(Component(id="C3", category="Cabin", name="C3", unit="pcs"))
    registry._data.append(Dependency(id="D2", source_id="C2", target_id="C3", dependency_type=DependencyType.REQUIRES))
