...
from app.pricing_engine.validator import PricingValidator, PricingValidationError
...
def test_duplicate_pricing_records_fail_fast():
    catalogue = PricingCatalogue(
        ...
        pricing_records=[
            PricingRecord(entity_id="COMP_1", entity_type="COMPONENT", price=Decimal("100")),
            PricingRecord(entity_id="COMP_1", entity_type="COMPONENT", price=Decimal("150")),
        ]
    )
    validator = PricingValidator()
    with pytest.raises(PricingValidationError) as exc:
...
def test_negative_price_fails_fast():
    catalogue = PricingCatalogue(
        ...
        tax_configuration=TaxConfiguration(enabled=True, tax_name="VAT", rate=Decimal("10.0")),
        pricing_records=[
            PricingRecord(entity_id="COMP_1", entity_type="COMPONENT", price=Decimal("-10.0")),
        ]
    )
    validator = PricingValidator()
    with pytest.raises(PricingValidationError) as exc:
...
def test_bom_unit_cost_sum_equals_component_cost():
    ...
    pricing_cat = PricingCatalogue(
        ...
        tax_configuration=TaxConfiguration(enabled=True, tax_name="VAT", rate=Decimal("10.0")),
        pricing_records=[
            PricingRecord(entity_id="CAT_A", entity_type="CATEGORY", price=Decimal("1000")),
            PricingRecord(entity_id="COMP_1", entity_type="COMPONENT", price=Decimal("100.55")),
            PricingRecord(entity_id="COMP_2", entity_type="COMPONENT", price=Decimal("200.45")),
        ]
    )
    ...
    sum_bom = sum(item.unit_cost for item in config.bill_of_materials.items if item.unit_cost is not None)
