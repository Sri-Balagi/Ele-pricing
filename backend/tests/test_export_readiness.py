...
def test_exporter_readiness():
    ...
    from app.models.domain import BillOfMaterials, PricingSummary
...
def test_configuration_immutability():
    config = Configuration(
        configuration_id="CFG-123", status=ConfigurationStatus.PRICED
    )
...
def test_pipeline_repeatability(client):
    ...
    if config_1.pricing_summary:
        assert (
            config_1.pricing_summary.total_after_tax
            == config_2.pricing_summary.total_after_tax
        )
    if config_1.bill_of_materials:
        assert (
            config_1.bill_of_materials.total_components
            == config_2.bill_of_materials.total_components
        )
