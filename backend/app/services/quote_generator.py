...
class QuoteGenerator:
    ...
    def generate(self, configuration: Configuration) -> None:
        ...
        valid_until_str = (datetime.now(timezone.utc) + timedelta(days=30)).isoformat()
        ...
        configuration.quote_metadata.quote_hash = self._generate_fingerprint(configuration)
    ...
    def _generate_fingerprint(self, configuration: Configuration) -> str:
        ...
        if configuration.pricing_summary:
            components.append(f"total:{float(configuration.pricing_summary.total_after_tax)}")
        if configuration.bill_of_materials:
            components.append(f"bom_size:{configuration.bill_of_materials.total_components}")
