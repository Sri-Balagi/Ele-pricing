...
from app.core.constants import ConfigurationStatus, ErrorCode, RuleTriggerType
from app.core.engine_base import BaseEngine
from app.core.exceptions import StartupValidationError
...
class ConfigurationPipeline:
    ...
    def execute(
        self, configuration: Configuration, request_source: str = "API"
    ) -> PipelineExecutionReport:
        """
        ...
        logger.info(
            f"[{correlation_id}] BeforePipeline: Executing for config {configuration.configuration_id}"
        )
        ...
        if any(not r.ready for r in startup_reports):
            logger.error(
                f"[{correlation_id}] Pipeline aborted due to startup validation failure."
            )
        ...
        for mapping in self.catalogue.mappings:
            if (
                mapping.active
                and mapping.feature_option_id in configuration.selected_feature_options
            ):
        ...
        try:
            ...
            report.errors.append(f"{ErrorCode.RULE_001}: Rule Engine Failed - {e!s}")
            pipeline_success = False

        if not pipeline_success:
            return self._finalize_pipeline(
                report, configuration, ConfigurationStatus.DRAFT, t0
            )
        ...
        try:
            ...
            if len(dep_report.cycles_detected) > 0:
                report.errors.append(
                    f"{ErrorCode.DEP_001}: Circular dependencies detected."
                )
                pipeline_success = False
                logger.error(
                    f"[{correlation_id}] AfterEngine: DependencyEngine (Failed - Cycle Detected)"
                )
            else:
                logger.info(
                    f"[{correlation_id}] AfterEngine: DependencyEngine (Success)"
                )
        except Exception as e:
            logger.error(f"[{correlation_id}] Dependency Engine failed: {e}")
            report.errors.append(
                f"{ErrorCode.DEP_001}: Dependency Engine Failed - {e!s}"
            )
            pipeline_success = False
        ...
        try:
            ...
            if len(pricing_report.errors) > 0:
                ...
                pipeline_success = False
                logger.error(
                    f"[{correlation_id}] AfterEngine: PricingEngine (Failed - Missing Prices)"
                )
            else:
            ...
            report.errors.append(
                f"{ErrorCode.PRICE_001}: Pricing Engine Failed - {e!s}"
            )
            pipeline_success = False

        if not pipeline_success:
            return self._finalize_pipeline(
                report, configuration, ConfigurationStatus.VALIDATED, t0
            )
    ...
    def _finalize_pipeline(
        ...
        report: PipelineExecutionReport,
        configuration: Configuration,
        final_status: ConfigurationStatus,
        start_time: float,
    ) -> PipelineExecutionReport:
        ...
        report.metrics.total_execution_time_ms = (
        ...
        configuration.validation_results = ValidationResult(
            is_valid=report.metrics.success, errors=errors, warnings=[], info=[]
        )

        logger_msg = "PipelineSucceeded" if report.metrics.success else "PipelineFailed"
        logger.info(
            f"[{report.correlation_id}] {logger_msg}: Completed in {report.metrics.total_execution_time_ms:.2f}ms with status {final_status}"
        )
