...
class CatalogueValidationException(Exception):
    ...
    def __init__(self, errors: list[str]):
        self.errors = errors
        super().__init__(
            f"Catalogue validation failed with {len(errors)} errors:\n- "
            + "\n- ".join(errors)
        )
...
class CatalogueValidator:
    ...
    def _load_data(self) -> None:
        ...
        def build_dict(
            data_list: list[dict[str, Any]], entity_name: str
        ) -> dict[str, Any]:
            result = {}
            for item in data_list:
                ...
                if item_id in result:
                    self.errors.append(
                        f"Duplicate ID found for {entity_name}: {item_id}"
                    )
                else:
                    result[item_id] = item
            return result
    ...
    def _validate_pydantic_schemas(self) -> None:
        ...
        def validate_dict(model, data_dict: dict[str, Any], entity_name: str) -> None:
            for item_id, item in data_dict.items():
                try:
                    ...
                    self.errors.append(
                        f"Schema validation failed for {entity_name} '{item_id}': {exc}"
                    )
        ...
        if isinstance(meta_raw, list) and len(meta_raw) > 0:
            try:
                ...
                self.errors.append(
                    f"Schema validation failed for CatalogMetadata: {exc}"
                )
        ...
        validate_dict(
            FeatureComponentMapping, self._mappings, "FeatureComponentMapping"
        )
    ...
    def _validate_references(self) -> None:
        ...
        for f_id, f in self._features.items():
            ...
            if cat_id not in self._categories:
                self.errors.append(
                    f"Feature '{f_id}' references unknown category_id: {cat_id}"
                )
            else:
                used_categories.add(cat_id)
                if not self._categories[cat_id].get("active", True):
                    self.errors.append(
                        f"Feature '{f_id}' references inactive category_id: {cat_id}"
                    )

            grp_id = f.get("group_id")
            if grp_id not in self._feature_groups:
                self.errors.append(
                    f"Feature '{f_id}' references unknown group_id: {grp_id}"
                )
            else:
        ...
        for opt_id, opt in self._feature_options.items():
            ...
            if f_id not in self._features:
                self.errors.append(
                    f"FeatureOption '{opt_id}' references unknown feature_id: {f_id}"
                )
            else:
                used_features.add(f_id)
                if not self._features[f_id].get("active", True):
                    self.errors.append(
                        f"FeatureOption '{opt_id}' references inactive feature_id: {f_id}"
                    )
        ...
        for m_id, m in self._mappings.items():
            ...
            if opt_id not in self._feature_options:
                self.errors.append(
                    f"Mapping '{m_id}' references unknown feature_option_id: {opt_id}"
                )
            else:
            ...
            if comp_id not in self._components:
                self.errors.append(
                    f"Mapping '{m_id}' references unknown component_id: {comp_id}"
                )
            else:
        ...
        for d_id, d in self._dependencies.items():
            ...
            if src_id not in self._components and src_id not in self._feature_options:
                self.errors.append(
                    f"Dependency '{d_id}' references unknown source_id: {src_id}"
                )
            else:
            ...
            if tgt_id not in self._components and tgt_id not in self._feature_options:
                self.errors.append(
                    f"Dependency '{d_id}' references unknown target_id: {tgt_id}"
                )
            else:
        ...
        if orphaned_features:
            logger.warning(
                "Found %d orphaned Features with no FeatureOptions.",
                len(orphaned_features),
            )

        orphaned_components = set(self._components.keys()) - used_components
        if orphaned_components:
            logger.warning(
                "Found %d orphaned Components with no mappings or dependencies.",
    ...
    def validate(self) -> None:
        ...
        try:
            ...
            self.errors.append(f"Failed to load data for validation: {e!s}")
        ...
        if self.errors:
            logger.error(
                "Catalogue validation failed with %d errors.", len(self.errors)
            )
