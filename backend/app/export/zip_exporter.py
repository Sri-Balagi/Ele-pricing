...
from app.models.domain import ExportContext, ExportManifest, ExportReport
...
class ZIPExporter(BaseExporter):
    ...
    def export(self, context: ExportContext) -> ExportReport:
        ...
        if (
            not json_report.success
            or not pdf_report.success
            or not excel_report.success
        ):
            return ExportReport(
                ...
                success=False,
                warnings=[
                    "Failed to generate one or more artifacts for the ZIP bundle."
                ],
            )
        ...
        manifest = ExportManifest(
            ...
            quote_number=context.configuration.quote_metadata.quote_number
            if context.configuration.quote_metadata
            else None,
            exported_files=[
