...
class BOMCostResolver:
    ...
    def populate_unit_costs(
        self,
        context: PricingContext,
        category_cost: Decimal,
        floor_cost: Decimal,
        feature_cost: Decimal,
    ) -> int:
        """
        ...
        if feature_components_raw_total > 0:
            ...
            feature_scale_factor = target_feature_total / max(
                Decimal("1.0"), Decimal(len(feature_items))
            )
        ...
        for item in context.configuration.bill_of_materials.items:
            ...
            if origin_val == "BASE":
                if record:
                    ...
                    item.unit_cost = scaled_cost.quantize(
                ...
                if record:
                    ...
                    if feature_components_raw_total == 0 and target_feature_total > 0:
                        item.unit_cost = feature_scale_factor.quantize(
                            Decimal("0.01"), rounding=ROUND_HALF_UP
                        )
                    else:
                        item.unit_cost = Decimal("0.00")
        ...
        if base_items:
            current_sum = sum(
                (i.unit_cost for i in base_items if i.unit_cost is not None),
                Decimal("0.00"),
            )
        ...
        if feature_items:
            current_sum = sum(
                (i.unit_cost for i in feature_items if i.unit_cost is not None),
                Decimal("0.00"),
            )
