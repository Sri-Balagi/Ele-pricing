...
class PricingCalculator:
    ...
    def calculate_category_cost(
        self, context: PricingContext, step_counter: int
    ) -> tuple[Decimal, PricingStep]:
        ...
        if not category_id:
            raise PricingCalculationError(
                "No category selected in configuration.", "UNKNOWN"
            )

        record = context.pricing_registry.get_pricing_record(category_id)
        if not record:
            raise PricingCalculationError(
                f"Missing pricing record for category: {category_id}", category_id
            )
    ...
    def calculate_feature_costs(
        self, context: PricingContext, start_step: int
    ) -> tuple[Decimal, list[PricingStep]]:
        ...
        for option_id in context.configuration.selected_feature_options:
            ...
            if not record:
                ...
                raise PricingCalculationError(
                    f"Missing pricing record for feature option: {option_id}", option_id
                )
    ...
    def calculate_component_costs(
        ...
        for comp_id in context.configuration.resolved_components:
            ...
            if not record:
                raise PricingCalculationError(
                    f"Missing pricing record for component: {comp_id}", comp_id
                )
    ...
    def calculate_floor_coverage_cost(
        self, context: PricingContext, start_step: int
    ) -> tuple[Decimal, PricingStep | None]:
